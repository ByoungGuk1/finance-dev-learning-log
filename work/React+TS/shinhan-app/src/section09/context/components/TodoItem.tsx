...
const TodoItem = ({ id, content, isDone, createdDate, writer }: todoType) => {
  const { onUpdate, onDelete } = useContext(TodoDispatchContext);
  const [updateContent, setUpdateContent] = useState(content);
  const [updateWriter, setUpdateWriter] = useState(writer);
  ...
  useEffect(() => {
    ...
    if (debouncedWriter?.trim()) {
      onUpdate(id, "writer", debouncedWriter!);
    }
  }, [debouncedContent, debouncedWriter, id, onUpdate]);
  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-white border border-gray-200 rounded-lg hover:shadow-md transition-shadow">
      ...
      <div className="flex-1 flex gap-2">
        ...
        <div className="self-center text-sm text-gray-500 min-w-[100px]">
