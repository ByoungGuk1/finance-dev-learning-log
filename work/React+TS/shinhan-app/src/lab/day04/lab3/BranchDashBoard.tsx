...
const BranchDashBoard = () => {
  ...
  const toggleBranch = (name: string): void => {
    setBranches((prev) => [
      ...prev.map((data) => {
        if (data.name === name) return { ...data, open: !data.open };
        return { ...data };
      }),
    ]);
  ...
  const openCount: number = useMemo(() => {
    return branches.filter((data) => data.open === true).length;
  }, [branches]); // 이 줄을 useMemo 코드로 교체하세요.
