...
function accountReducer(state: State, action: Action): State {
  switch (action.type) {
    case "DEPOSIT":
      state.history.push(`입금 ${action.amount}`);
      return { ...state, balance: state.balance + action.amount };
    case "WITHDRAW":
      if (state.balance < action.amount) return state;
      state.history.push(`출금 ${action.amount}`);
...
const AccountTransaction = () => {
  ...
  const handleDeposit = (): void => {
    if (Number(depositInput) > 0) {
      dispatch({ type: "DEPOSIT", amount: Number(depositInput) });
      setDepositInput("");
    }
  };
  ...
  const handleWithdraw = (): void => {
    if (Number(withdrawInput) > 0) {
      dispatch({ type: "WITHDRAW", amount: Number(withdrawInput) });
      setWithdrawInput("");
    }
  };
