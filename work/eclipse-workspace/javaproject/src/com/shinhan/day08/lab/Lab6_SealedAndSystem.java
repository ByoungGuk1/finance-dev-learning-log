...
/**
 * =====================================================
 * [고급 LAB 6] sealed 클래스 + 7장 종합 — 금융 이벤트 시스템
 * 모듈: Module 02 - 7장 상속
 * 난이도: ★★★
 * 목표: 1. sealed 클래스로 상속 범위 제한 (Java 17+)
 *       2. final / non-sealed 자식 클래스 설계
 *       3. 추상 클래스 + 템플릿 메서드 패턴 적용
 *       4. instanceof 패턴매칭으로 타입 안전 처리
 ...
 *
 * sealed 계층:
 *   BankEvent (sealed 추상)      — permits 3개만 허용
 *   ├── DepositEvent  (final)    — 입금 이벤트 (더 이상 상속 불가)
 *   ├── WithdrawEvent (final)    — 출금 이벤트 (더 이상 상속 불가)
 *   └── TransferEvent (non-sealed) — 이체 이벤트 (추가 확장 허용)
 *       └── InternationalTransfer  — 해외송금
 *
 *   EventProcessor (추상)        — 이벤트 처리기 (템플릿 메서드) 순서 고정: canProcess → doProcess → afterProcess
 *   ├── AuditProcessor (final)   — 감사 처리기 (모든 이벤트를 기록하고 위험 감지, 감사 로직을 자식이 변경하면 안 됨 (보안))
 *   └── AlertProcessor           — 알림 처리기 (기준금액 이상일 때만 알림 발송,canProcess() 오버라이딩으로 필터링 )
 *
